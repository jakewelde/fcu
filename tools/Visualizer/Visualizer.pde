...
void serialEvent()
{
  ...
  do {
    ...
    if (message != null) {
      ...
      if (list.length >= 16 && list[0].equals("Orientation:")) {
        ...
        M[0]    = 10*float(list[13]); // convert to float yaw
        M[1]    = 10*float(list[14]); // convert to float yaw
        M[2]    = 10*float(list[15]); // convert to float yaw
      }
    }
  } while (message != null);
}
